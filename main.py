...
# Render.com предоставляет порт через переменную окружения PORT и ожидает,
# что веб-сервис начнёт его слушать вскоре после старта, иначе деплой
# считается неудачным ("порт не найден"). Бот сам по себе HTTP не раздаёт
# (только Telegram polling), поэтому поднимаем простой health-check сервер.
...
# --------------------------------------------------------------------------- #
# HTTP health-check сервер (нужен только для Render.com Web Service)
# --------------------------------------------------------------------------- #

class _HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802 (имя метода задано base-классом)
        body = b"OK"
    ...
    def log_message(self, format: str, *args) -> None:  # подавляем шумные логи
        pass
...
def start_health_server(port: int) -> None:
    def _run() -> None:
        try:
            ...
            logger.exception("Не удалось запустить health-check сервер на порту %d", port)

    threading.Thread(target=_run, daemon=True, name="health-check-server").start()
...
def fetch_category(session: requests.Session, slug: str, rarity_label: str) -> list[Item]:
    ...
    # Реальная разметка supremevalues.com — это таблица строк (tr), где в
    # одной ячейке лежит <img alt="Имя предмета" src="...">, а в соседней —
    # свободный текст вида:
    #   Traveler's Gun
    #   Value - 5,200 ... Ranged Value - [5,200 - 5,400]
    #   Stability - Doing Well
    #   Demand - 6  Rarity - 5
    #   Origin - Hallows 2023 (Unboxed)
    # Никаких классов itemcolumn/itemimage/itemvalue или data-* атрибутов
    # на сайте нет — старый парсер их просто не находил, поэтому кэш
    # наполнялся пустыми/N-A предметами. Вместо жёсткой привязки к тегам
    # ищем по картинке (она однозначно определяет один предмет) и берём
    # текст ближайшего общего блока-контейнера (обычно <tr> или <td>).
    img_tags = soup.find_all("img", alt=True)

    for img_tag in img_tags:
        alt_name = (img_tag.get("alt") or "").strip()
        if not alt_name:
            continue
        # Служебные картинки (иконки стабильности, значки, шапки разделов)
        # не являются предметами — пропускаем по типичным путям.
        src_check = img_tag.get("src", "")
        if "/media/stability/" in src_check or "/media/icons/" in src_check \
                or "/media/headers/" in src_check or "experimental" in src_check \
                or "badge" in src_check.lower():
            continue

        # Находим ближайший контейнер-строку, содержащий текст с "Value".
        container = img_tag.find_parent("tr")
        if container is None:
            container = img_tag.find_parent("td")
        if container is None:
            container = img_tag.parent
        # Поднимаемся вверх, пока не найдём блок, где есть слово "Value"
        hops = 0
        node = container
        while node is not None and "Value" not in node.get_text() and hops < 4:
            node = node.parent
            hops += 1
        if node is not None:
            container = node
        ...
        value_match = re.search(r"Value\s*-\s*([\d,]+|N/A)", block_text)
        ...
        ranged_match = re.search(r"Ranged Value\s*-\s*\[([^\]]+)\]", block_text)
        ranged_value = ranged_match.group(1).strip() if ranged_match else None
        if ranged_value and ranged_value.upper() == "N/A":
            ranged_value = None

        stability_match = re.search(
            r"Stability\s*-\s*([A-Za-z' ]+?)(?=\s+Demand|\s+Rarity|\s+Origin|$)",
            block_text,
        )
        stability = stability_match.group(1).strip() if stability_match else "Неизвестно"

        origin_match = re.search(
            r"Origin\s*-\s*(.+?)(?=\s+Last Change|$)", block_text
        )
        ...
        if src:
            if src.startswith("//"):
                ...
                image_url = f"{BASE_URL}/{src.lstrip('/')}"
...
def download_image(url: str) -> Optional[Image.Image]:
    if not url:
        return None
    try:
        resp = requests.get(url, timeout=10, headers=_IMAGE_REQUEST_HEADERS)
        content_type = resp.headers.get("Content-Type", "")
        if resp.status_code == 200 and content_type.startswith("image/"):
            return Image.open(io.BytesIO(resp.content)).convert("RGBA")
        logger.warning(
            "Изображение %s вернуло статус %d / content-type '%s'",
...
def format_item_caption(item: Item, lang: str) -> str:
    ...
    if item.ranged_value:
        lines.append(f"Range: <b>{html.escape(item.ranged_value)}</b>")
    lines += [
        "",
...
def main() -> None:
    # Запускаем HTTP-сервер сразу, чтобы Render как можно раньше увидел
    # открытый порт и не считал деплой неудачным по таймауту.
